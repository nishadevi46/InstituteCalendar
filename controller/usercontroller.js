...
export const loginUser=async(request,response)=>{
    ...
    if(!user){
        return response.status(400).json({msg:'Username doest not match'})
    }
    try {
       ...
       if(match){
             ...
             await newToken.save()
             return response.status(200).json({accessToken:accessToken,refreshToken:refreshToken,name:user.name,username:user.username,email:user.email})
       }
       else{
